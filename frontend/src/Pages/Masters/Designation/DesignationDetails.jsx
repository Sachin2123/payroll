...
const fetchDesignation = async () => {
  const result = await fetch("http://localhost:5000/api/designationdetails");
  if (!result.ok) throw new Error("error in fetching designation data");
  // console.log(result);
  return result.json();
};
      ...
      <Paper elevation={3} sx={{ height: "85vh", overflowY: "auto" }}>
        ...
        <Box sx={{ display: "flex", p: 2, justifyContent: "space-between" }}>
          ...
          <Box sx={{ display: "flex", gap: 2 }}>
            ...
            <Button
              ...
              onClick={() => navigate("/export-designation")}
              sx={{
                boxShadow: "2px 2px 2px 1px rgba(0, 0, 255, .2)",
                color: "white",
                background: "black",
        ...
        <Box sx={{ mt: 8, mb: 1, display: "flex", justifyContent: "center" }}>
          ...
          <DataGrid
            rows={
              data
                ? data.map((row) => ({ ...row, id: row.Designation_ID }))
                : []
            }
            // rows={data}
            columns={columns}
